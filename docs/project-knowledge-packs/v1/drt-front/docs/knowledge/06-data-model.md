...
- Pinia stores under `src/store/*.ts` are client state carriers.
- `DrtHttpResponse<T>` defines common response fields.
- Payload changes must trace view -> store/service -> backend contract.
