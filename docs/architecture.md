...
- Next.js UI
  ...
  - Kiwi 메인 세션 프롬프트 입력과 Prompt Builder 전송
  - FAST/lightwork, ultrawork, superpowers work mode 선택
  - ultrawork/superpowers 사용자 선택 티셔츠 사이즈 관리
  - Prompt Builder 우측 패널에서 실행 전 요구사항 인터뷰, LangGraph activity, 선택 work mode별 최종 프롬프트 표시
...
- LangGraph
  ...
  - 실행 전 Prompt Builder에서 Qwen3.5가 의도 분석, KK docs MCP/로컬 검색 계획, 인터뷰 질문, 선택 work mode별 표준 프롬프트 생성을 수행하는 보조 워크플로로 사용한다.

- Documentation Harness
  - 대형 프로젝트를 수정하기 전에 `D:/aiops/docs/<project-key>/knowledge/` 지식 베이스를 만들기 위한 표준 절차와 산출물 계약이다.
...
6. Prompt Builder는 `KIWI.md`, 중앙 Project Docs, 선택적 Project Info Layer, KK docs MCP 근거, 선택된 프로젝트 루트 내부 검색 계획을 합쳐 최종 work mode 프롬프트를 만든다.
...
4. 문서화 산출물은 중앙 문서 루트 `D:/aiops/docs/<project-key>/knowledge/` 아래에 생성된다.
5. 선택적 Project Info Layer는 같은 중앙 루트의 `D:/aiops/docs/<project-key>/project-info/` 아래에 생성된다.
6. target 프로젝트의 `KIWI.md`에는 상세 내용을 복사하지 않고 knowledge base 인덱스와 핵심 사용법만 요약한다.
7. 이후 Prompt Builder와 벡터DB ingest는 `D:/aiops/docs/<project-key>/knowledge/`의 front matter, keyword, evidence, cross-link를 사용해 관련 문서를 검색한다.
