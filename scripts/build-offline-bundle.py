...
KIWI_ZIP_NAME = "kiwi-offline-win11-py313"
KIWI_ROOT_NAME = "kiwi"
QWENCODE_ZIP_NAME = "qwencode-win11-v0.17.1"
QWENCODE_ROOT_NAME = "qwencode"
BUNDLE_ROOT = OUT_DIR / KIWI_ROOT_NAME
QWENCODE_BUNDLE_ROOT = OUT_DIR / QWENCODE_ROOT_NAME
...
DEFAULT_QWENCODE_RUNTIME_DIR = r"D:\aiops\qwencode"
...
def main() -> int:
    ...
    print(f"Using external Qwen runtime: {runtime}")
    prepare_external_qwen_runtime(runtime)
    copy_qwencode_bundle(runtime, QWENCODE_BUNDLE_ROOT)
    reset_dir(BUNDLE_ROOT)
    copy_source(BUNDLE_ROOT)
    write_frontend_build_stamp(BUNDLE_ROOT)
    ...
    for root, zip_name in [
        (QWENCODE_BUNDLE_ROOT, QWENCODE_ZIP_NAME),
        (BUNDLE_ROOT, KIWI_ZIP_NAME),
    ]:
...
def write_frontend_build_stamp(target_root: Path) -> None:
    digest = hashlib.sha256()
    for relative in FRONTEND_BUILD_STAMP_INPUTS:
        source = ROOT / relative
        if not source.exists():
            continue
        if source.is_file():
            update_stamp_digest(digest, source, relative)
            continue
        for path in sorted(p for p in source.rglob("*") if p.is_file()):
            if should_ignore_stamp_path(path):
                continue
            update_stamp_digest(digest, path, str(path.relative_to(ROOT)).replace(os.sep, "/"))
    (target_root / "kiwi-source-stamp.txt").write_text(digest.hexdigest() + "\n", encoding="utf-8")


def should_ignore_stamp_path(path: Path) -> bool:
    parts = set(path.relative_to(ROOT).parts)
    return bool(parts & SOURCE_EXCLUDE_DIRS) or path.name.endswith(".pyc") or path.name == "tsconfig.tsbuildinfo"


def update_stamp_digest(digest, path: Path, relative: str) -> None:
...
def prepare_external_qwen_runtime(runtime_root: Path) -> None:
    print("Preparing external Qwen runtime source...")
    normalize_qwencode_runtime_layout(runtime_root)
    install_yarn_classic_runtime(runtime_root)
    write_qwen_runtime_config(runtime_root)
    write_qwen_runtime_aliases(runtime_root)
    patch_qwen_runtime_launchers(runtime_root)
    patch_qwen_init_script(runtime_root)
    patch_qwen_runtime_policy(runtime_root)
    patch_qwen35_vision_runtime(runtime_root)
    generate_qwen_runtime_config(runtime_root)
    patch_qwen_settings_files(runtime_root)
    install_kiwi_fast_system_prompts(runtime_root)
    install_kiwi_ultrawork_agents(runtime_root)
    install_kiwi_ultrawork_policy(runtime_root)
    install_kiwi_superpowers_extension(runtime_root)


def copy_qwencode_bundle(runtime: Path, target_root: Path) -> None:
    print("Copying standalone qwencode package...")
    reset_dir(target_root)
    for item in runtime.iterdir():
        if item.name in {"portable-runtime"} or item.name.endswith(".log"):
            continue
        dst = target_root / item.name
        if item.is_dir():
            shutil.copytree(item, dst, ignore=shutil.ignore_patterns("*.log"))
        else:
            shutil.copy2(item, dst)
    normalize_qwencode_runtime_layout(target_root)
    install_yarn_classic_runtime(target_root)
    write_qwencode_package_files(target_root)


def write_qwencode_package_files(runtime_root: Path) -> None:
    write_crlf(
        runtime_root / "install-path.cmd",
        rf"""
@echo off
...
if not exist "%ROOT%\run-qwen.cmd" (
  echo [ERROR] run-qwen.cmd not found in %ROOT%
  exit /b 1
)
if not exist "%ROOT%\qwen-init.cmd" (
  echo [ERROR] qwen-init.cmd not found in %ROOT%
  exit /b 1
)
if not exist "%ROOT%\runtimes\node\npm.cmd" (
  echo [ERROR] bundled Node/npm not found in %ROOT%\runtimes\node
  exit /b 1
)
if not exist "%ROOT%\runtimes\node\yarn.cmd" (
  echo [ERROR] bundled Yarn classic not found in %ROOT%\runtimes\node
  exit /b 1
)
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$parts=@(); if ($path) {{ $parts=$path -split ';' | Where-Object {{ $_ }}; }}" ^
  "if (-not ($parts | Where-Object {{ $_ -ieq $target }})) {{ $newPath=(($parts + $target) -join ';'); [Environment]::SetEnvironmentVariable('Path',$newPath,'User'); Write-Host ('[PATH] Added ' + $target + ' to user PATH'); }}" ^
  "[Environment]::SetEnvironmentVariable('KIWI_QWENCODE_RUNTIME_DIR',$target,'User');"
if errorlevel 1 exit /b %ERRORLEVEL%

echo.
echo qwencode runtime ready: %ROOT%
echo Reopen terminal, then qwen-init.cmd, qwen.cmd, and qwencode.cmd are available from any project path.
""".strip()
        + "\n",
    )
    write_crlf(
        runtime_root / "README_QWENCODE.md",
        rf"""
# qwencode standalone runtime

이 폴더는 폐쇄망 Windows 11 반입용 Qwen Code/qwencode standalone runtime이다.

## 포함 상태

- Qwen Code: 0.17.1
- Runtime root: `D:\aiops\qwencode`
- Main model: `{ORCHESTRATOR_MODEL}`
- Coder model: `{CODER_MODEL}`
- Vision config: `modalities.image=true`, `splitToolMedia=true`
- Work modes: `lightwork`, `ultrawork`, `superpowers`
- Superpowers skills: `portable-user\.qwen\skills\*`, `templates\project\.qwen\skills\*`, `portable-user\.qwen\extensions\superpowers\skills\*`
- FAST system prompts: `portable-user\.qwen\extensions\fast-system-prompts`
- Managed runtimes: `runtimes\node` for Qwen/DRT/Playwright Node 22, `runtimes\node10` for dcp-front Node 10, `runtimes\java` for Java 17 JDK, `runtimes\java8` for Java 8 JDK, `runtimes\maven3.6.3` for Maven 3.6.3, `runtimes\tomcat9` for Tomcat 9.0.115.
- Yarn classic: `runtimes\node\yarn.cmd` runs bundled Yarn {YARN_CLASSIC_VERSION} directly and does not rely on Corepack downloads.

## 설치

`qwencode-win11-v0.17.1.zip`을 `D:\aiops`에 풀어 `D:\aiops\qwencode`가 되게 한다.
...
`lightwork`, `ultrawork`, `superpowers` prefix는 KIWI 웹 없이 standalone qwen 세션에서도 동일하게 동작한다.
""".strip()
        + "\n",
    )


def generate_qwen_runtime_config(runtime_root: Path) -> None:
    script = runtime_root / "scripts" / "write-runtime-config.js"
    if not script.exists():
        return
    print("Generating Qwen ultrawork agent config...")
    env = os.environ.copy()
    env.update(
        {
            "QWEN_BUNDLE_ROOT": str(runtime_root),
            "QWEN_HOME": str(runtime_root / "portable-user" / ".qwen"),
            "QWEN35_API_KEY": "sk-local-qwen35",
            "QWEN35_BASE_URL": ORCHESTRATOR_API_BASE_URL,
            "QWEN35_MODEL": ORCHESTRATOR_MODEL,
            "CODER_API_KEY": "sk-local-coder",
            "CODER_BASE_URL": CODER_API_BASE_URL,
...
def write_qwen_runtime_aliases(runtime_root: Path) -> None:
    qwen_alias = r"""
@echo off
setlocal EnableExtensions
call "%~dp0run-qwen.cmd" %*
exit /b %ERRORLEVEL%
""".strip() + "\n"
    for name in ["qwen.cmd", "qwencode.cmd"]:
        write_crlf(runtime_root / name, qwen_alias)


def apply_qwen_console_ui_defaults(data: dict) -> None:
    ui = data.get("ui")
...
def patch_qwen_settings_files(runtime_root: Path) -> None:
    for settings_path in [
        runtime_root / "portable-user" / ".qwen" / "settings.json",
        runtime_root / "templates" / "project" / ".qwen" / "settings.json",
    ]:
        ...
        data["tools"] = tools
        apply_qwen_console_ui_defaults(data)
        settings_path.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def patch_qwen_runtime_launchers(runtime_root: Path) -> None:
    for relative in [Path("run-qwen.cmd"), Path("run-qwen-split.cmd")]:
        path = runtime_root / relative
        if not path.exists():
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        updated = rewrite_qwen_node_runtime_paths(text)
        ...
        if "KIWI forced no sandbox for Windows closed-network runtime" not in updated and anchor in updated:
            ...
            updated = updated.replace(anchor, insertion, 1)
        project_asset_sync = (
        ...
        if updated != text:
            write_crlf(path, updated)


def normalize_qwencode_runtime_layout(runtime_root: Path) -> None:
    runtimes = runtime_root / "runtimes"
    node_runtime = runtimes / "node"
    legacy_node = runtime_root / "node"
...
def install_yarn_classic_runtime(runtime_root: Path) -> None:
    node_runtime = runtime_root / "runtimes" / "node"
    if not (node_runtime / "node.exe").exists():
        return
    yarn_dir = node_runtime / "node_modules" / "yarn"
    package_json = yarn_dir / "package.json"
    current_version = ""
    if package_json.exists():
        try:
            ...
            current_version = ""
    if current_version != YARN_CLASSIC_VERSION:
        print(f"Installing bundled Yarn classic {YARN_CLASSIC_VERSION}...")
        if yarn_dir.exists():
            shutil.rmtree(yarn_dir)
        yarn_dir.parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix="kiwi-yarn-classic-") as raw_tmp:
            tmp = Path(raw_tmp)
            run(["npm", "pack", f"yarn@{YARN_CLASSIC_VERSION}", "--pack-destination", str(tmp)])
            tarballs = sorted(tmp.glob("yarn-*.tgz"))
            if not tarballs:
                raise RuntimeError("npm pack did not produce a Yarn tarball")
            ...
            shutil.copytree(tmp / "package", yarn_dir)

    yarn_cmd = r"""
@echo off
setlocal EnableExtensions
"%~dp0node.exe" "%~dp0node_modules\yarn\bin\yarn.js" %*
exit /b %ERRORLEVEL%
""".strip() + "\n"
    for name in ["yarn.cmd", "yarnpkg.cmd"]:
        write_crlf(node_runtime / name, yarn_cmd)
    yarn_sh = """#!/bin/sh
DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
exec "$DIR/node" "$DIR/node_modules/yarn/bin/yarn.js" "$@"
"""
    for name in ["yarn", "yarnpkg"]:
        path = node_runtime / name
        path.write_text(yarn_sh, encoding="utf-8")
        path.chmod(0o755)


def rewrite_qwen_node_runtime_paths(text: str) -> str:
    return (
        text.replace("%ROOT%\\node\\", "%ROOT%\\runtimes\\node\\")
        .replace("%ROOT%\\node;", "%ROOT%\\runtimes\\node;")
...
def patch_qwen_init_script(runtime_root: Path) -> None:
    init_path = runtime_root / "scripts" / "qwen-init.ps1"
...
def patch_qwen_runtime_policy(runtime_root: Path) -> None:
    ...
    if cli_path.exists():
        original_cli = cli_path.read_text(encoding="utf-8", errors="replace")
        cli = normalize_runtime_agent_names(original_cli)
        cli = patch_qwen_cli_hooks(cli)
        cli = patch_core_tool_prompt_examples(cli)
        cli = patch_edit_tool_failure_guidance(cli)
        cli = patch_console_paste_guard(cli)
        cli_changed = cli != original_cli
        ...
        if (["planner-35", "architect-35", "reviewer-35", "debugger-35", "tester-35", "explorer-35"].includes(subagentName)) {
    ...
    chunks_dir = runtime_root / "app" / "chunks"
    if chunks_dir.exists():
        for chunk in sorted(chunks_dir.glob("*.js")):
            original_chunk = chunk.read_text(encoding="utf-8", errors="replace")
            patched_chunk = patch_qwen_cli_hooks(original_chunk)
            patched_chunk = patch_edit_tool_failure_guidance(patched_chunk)
            if patched_chunk != original_chunk:
                chunk.write_text(patched_chunk, encoding="utf-8")
    ...
    policy = patch_work_mode_policy_script(policy)
    policy = policy.replace("tester-next", "tester-35")
    policy_path.write_text(policy, encoding="utf-8")

    activation_path = runtime_root / "scripts" / "ultrawork-activate.js"
    if activation_path.exists():
        activation = activation_path.read_text(encoding="utf-8", errors="replace")
        activation = patch_work_mode_activation_script(activation)
    ...
    if config_path.exists():
        ...
        config = patch_write_runtime_config_custom_agents(config)
        ...
        config = config.replace(
            ...
            'name: "Qwen3.5 Explorer"',
        )
        config = config.replace(
            'name: "Qwen3.5 Coder / Tester / Explorer"',
            'name: "Qwen3.5 Explorer"',
        )
        config = config.replace(
        ...
        config = config.replace(
            'writeText(path.join(extensionDir, "agents", "explorer-35.md"), agent(\n    coderModel,',
            'writeText(path.join(extensionDir, "agents", "explorer-35.md"), agent(\n    qwen35Model,',
        )
        config = config.replace(
    ...
    if team_log_path.exists():
        ...
        team_log = patch_work_mode_state_script(team_log)
        team_log = patch_ultrawork_activation_message(team_log)
...
def patch_qwen35_vision_runtime(runtime_root: Path) -> None:
    """Force Samsung Life Qwen3.5-397B to be treated as image-capable."""
    paths = [
        runtime_root / "scripts" / "write-runtime-config.js",
        runtime_root / "app" / "cli.js",
    ]
    chunks_dir = runtime_root / "app" / "chunks"
    if chunks_dir.exists():
        paths.extend(sorted(chunks_dir.glob("*.js")))
    for path in paths:
        if path.exists():
            patch_qwen35_vision_text_file(path)


def patch_qwen35_vision_text_file(path: Path) -> None:
    text = path.read_text(encoding="utf-8", errors="replace")
    updated = patch_qwen35_vision_text(text)
    if updated != text:
        path.write_text(updated, encoding="utf-8")


def patch_qwen35_vision_text(text: str) -> str:
    text = text.replace(
...
def install_kiwi_ultrawork_agents(runtime_root: Path) -> None:
    source_dir = ROOT / "docs" / "ultrawork-agents"
    sources: dict[str, Path] = {}
    for seed_dir in [
        runtime_root / "portable-user" / ".qwen" / "extensions" / "ultrawork" / "agents",
        runtime_root / "extensions" / "ultrawork" / "agents",
    ]:
    ...
    for agents_dir in [
        runtime_root / "portable-user" / ".qwen" / "extensions" / "ultrawork" / "agents",
        runtime_root / "extensions" / "ultrawork" / "agents",
        runtime_root / "portable-user" / ".qwen" / "agents",
        runtime_root / "templates" / "project" / ".qwen" / "agents",
    ]:
...
def install_kiwi_fast_system_prompts(runtime_root: Path) -> None:
    source_dir = ROOT / "docs" / "fast-system-prompts"
    if not source_dir.exists():
        return
    prompts = [path for path in source_dir.glob("*.md")]
    if not prompts:
        return
    for prompt_dir in [
...
def install_kiwi_ultrawork_policy(runtime_root: Path) -> None:
    source = ROOT / "docs" / "ultrawork-runtime-policy.md"
    if not source.exists():
        return
    policy = source.read_text(encoding="utf-8", errors="replace").strip()
    if not policy:
        return
    block_start = "<!-- KIWI:ULTRAWORK_POLICY:START -->"
    block_end = "<!-- KIWI:ULTRAWORK_POLICY:END -->"
    block = f"{block_start}\n{policy}\n{block_end}"
    pattern = re.compile(rf"{re.escape(block_start)}.*?{re.escape(block_end)}", flags=re.DOTALL)
    for extension_dir in [
        runtime_root / "portable-user" / ".qwen" / "extensions" / "ultrawork",
        runtime_root / "extensions" / "ultrawork",
    ]:
        ...
        qwen_path.write_text(updated, encoding="utf-8")


def install_kiwi_superpowers_extension(runtime_root: Path) -> None:
    source_dir = ROOT / "docs" / "superpowers-skills"
    if not source_dir.exists():
        return
    skill_dirs = [path for path in source_dir.iterdir() if (path / "SKILL.md").exists()]
    if not skill_dirs:
        return
    policy = ""
    policy_source = ROOT / "docs" / "superpowers-runtime-policy.md"
    if policy_source.exists():
        policy = policy_source.read_text(encoding="utf-8", errors="replace").strip()
    manifest = {
        "name": "superpowers",
        "version": "1.0.0",
...
def superpowers_skill_names() -> list[str]:
    source_dir = ROOT / "docs" / "superpowers-skills"
    if not source_dir.exists():
        return []
    return sorted(path.name for path in source_dir.iterdir() if (path / "SKILL.md").exists())


def normalize_runtime_agent_names(text: str) -> str:
    text = re.sub(r"(?<!qwen3-)coder-next", "coder-35", text)
    text = re.sub(r"(?<!Qwen3-)Coder-Next", "Coder-35", text)
    legacy_explorer = "explorer" + "-next"
    legacy_explorer_title = "Explorer" + "-next"
    text = text.replace(legacy_explorer, "explorer-35")
    text = text.replace(legacy_explorer_title, "Explorer-35")
    text = text.replace("Qwen3-Coder-Next", "Qwen3.5")
    return text


def patch_qwen_cli_hooks(text: str) -> str:
...
def patch_ultrawork_activation_message(text: str) -> str:
    text = text.replace(
...
def patch_work_mode_activation_script(text: str) -> str:
...
def patch_work_mode_state_script(text: str) -> str:
...
function projectDocsRuntimeLine(input) {
  ...
  writeState(state);
    ...
    text = replace_team_mode_context(text)
...
def replace_team_mode_context(text: str) -> str:
...
def patch_work_mode_policy_script(text: str) -> str:
...
def patch_core_tool_prompt_examples(text: str) -> str:
    replacements = {
...
def patch_edit_tool_description(text: str) -> str:
    replacements = {
...
def patch_edit_tool_span_repair(text: str) -> str:
...
def patch_console_paste_guard(text: str) -> str:
...
KIWI_PATH_RECOVERY_HELPER = r'''async function kiwiSuggestNearbyPath(requestedPath) {
  try {
...
KIWI_READ_NOT_FOUND_OLD = """      if (isNodeError(error) && error.code === "ENOENT") {
        return {
...
KIWI_READ_NOT_FOUND_PREV = """      if (isNodeError(error) && error.code === "ENOENT") {
...
KIWI_READ_NOT_FOUND_NEW = """      if (isNodeError(error) && error.code === "ENOENT") {
        const kiwiSuggestedPath = await kiwiSuggestNearbyPath(filePath);
        const kiwiPathHint = kiwiSuggestedPath
          ? ` KIWI path recovery: a nearly identical path exists at "${kiwiSuggestedPath}". The requested path looks like a re-typed file name (for example, extra spaces inserted around "-" or "_" in Korean file names). Call read_file again with file_path set to exactly "${kiwiSuggestedPath}", copied character-for-character, and reuse that exact path in later edit/write_file calls.`
...
KIWI_EDIT_NOT_FOUND_OLD = """    } else if (!fileExists2) {
      error = {
...
KIWI_EDIT_NOT_FOUND_NEW = """    } else if (!fileExists2) {
...
def patch_file_path_recovery_hints(text: str) -> str:
    ...
    """
    needs_read = KIWI_READ_NOT_FOUND_OLD in text
    needs_edit = KIWI_EDIT_NOT_FOUND_OLD in text
    needs_read_upgrade = KIWI_READ_NOT_FOUND_PREV in text
    ...
    if "kiwiSuggestNearbyPath" not in text:
        return text
    text = text.replace(KIWI_READ_NOT_FOUND_OLD, KIWI_READ_NOT_FOUND_NEW, 1)
    text = text.replace(KIWI_READ_NOT_FOUND_PREV, KIWI_READ_NOT_FOUND_NEW, 1)
    text = text.replace(KIWI_EDIT_NOT_FOUND_OLD, KIWI_EDIT_NOT_FOUND_NEW, 1)
    return text


def patch_edit_tool_failure_guidance(text: str) -> str:
    text = patch_edit_tool_description(text)
    text = patch_edit_tool_span_repair(text)
    text = patch_file_path_recovery_hints(text)
...
def patch_write_runtime_config_custom_agents(text: str) -> str:
    text = text.replace(
        """    tools: {
...
setlocal EnableExtensions
set "ROOT=%~dp0.."
set "RUNTIME=%KIWI_QWENCODE_RUNTIME_DIR%"
if "%RUNTIME%"=="" set "RUNTIME=D:\aiops\qwencode"
if not exist "%RUNTIME%\run-qwen.cmd" (
  echo [ERROR] External qwencode runtime not found: %RUNTIME%\run-qwen.cmd
  echo Copy the prepared qwencode runtime to D:\aiops\qwencode first, then reopen the terminal.
  exit /b 1
)
set "KIWI_QWENCODE_RUNTIME_DIR=%RUNTIME%"
call "%RUNTIME%\run-qwen.cmd" %*
exit /b %ERRORLEVEL%
""".strip()
    qwen_init_wrapper = r"""
@echo off
...
set "RUNTIME=%KIWI_QWENCODE_RUNTIME_DIR%"
if "%RUNTIME%"=="" set "RUNTIME=D:\aiops\qwencode"
if not exist "%RUNTIME%\qwen-init.cmd" (
  echo [ERROR] External qwencode runtime not found: %RUNTIME%\qwen-init.cmd
  echo Copy the prepared qwencode runtime to D:\aiops\qwencode first, then reopen the terminal.
  exit /b 1
)
set "KIWI_QWENCODE_RUNTIME_DIR=%RUNTIME%"
call "%RUNTIME%\qwen-init.cmd" %*
exit /b %ERRORLEVEL%
""".strip()
    root_qwen_wrapper = r"""
@echo off
...
""".strip()
    for name in ["qwen.cmd", "qwencode.cmd"]:
        write_crlf(
            target_root / "bin" / name,
            qwen_wrapper
            + "\n",
        )
    write_crlf(
        target_root / "bin" / "qwen-init.cmd",
        qwen_init_wrapper
        + "\n",
    )
    write_crlf(target_root / "qwen.cmd", root_qwen_wrapper + "\n")
    write_crlf(target_root / "qwen-init.cmd", root_qwen_init_wrapper + "\n")
    write_crlf(
        target_root / "check-qwencode-runtime.cmd",
        rf"""
@echo off
setlocal EnableExtensions
chcp 65001 >nul
set "TARGET=%KIWI_QWENCODE_RUNTIME_DIR%"
if "%TARGET%"=="" set "TARGET={DEFAULT_QWENCODE_RUNTIME_DIR}"
set "KIWI_QWENCODE_RUNTIME_DIR=%TARGET%"

if not exist "%TARGET%\run-qwen.cmd" (
  echo [ERROR] External qwencode runtime not found: %TARGET%\run-qwen.cmd
  echo Copy the prepared qwencode runtime to %TARGET% first.
  exit /b 1
)
if not exist "%TARGET%\qwen-init.cmd" (
  echo [ERROR] External qwencode runtime missing qwen-init.cmd: %TARGET%
  exit /b 1
)
if not exist "%TARGET%\runtimes\node\npm.cmd" (
  echo [ERROR] External qwencode runtime missing bundled Node/npm: %TARGET%\runtimes\node\npm.cmd
  exit /b 1
)
if not exist "%TARGET%\runtimes\node\yarn.cmd" (
  echo [ERROR] External qwencode runtime missing bundled Yarn classic: %TARGET%\runtimes\node\yarn.cmd
  exit /b 1
)
...
echo.
echo External qwencode runtime ready: %TARGET%
""".strip()
        + "\n",
    )
    write_crlf(
        target_root / "stop-kiwi.cmd",
        r"""
@echo off
setlocal EnableExtensions
chcp 65001 >nul
if "%KIWI_SKIP_PORT_CLEANUP%"=="1" (
  echo [INFO] KIWI port cleanup skipped because KIWI_SKIP_PORT_CLEANUP=1.
  exit /b 0
)
if "%KIWI_STOP_PORTS%"=="" set "KIWI_STOP_PORTS=3000,8787"
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  "$ports=$env:KIWI_STOP_PORTS -split ',' | ForEach-Object { [int]($_.Trim()) };" ^
  ...
  "  Write-Host ('[INFO] Stopping existing process on KIWI port: PID=' + $processId + ' ' + $cmd);" ^
  "  Stop-Process -Id $processId -Force -ErrorAction SilentlyContinue;" ^
  "}" ^
  "if ($pids.Count -gt 0) { Start-Sleep -Milliseconds 500 }"
exit /b 0
""".strip()
        + "\n",
    )
...
set "RUNTIME=%KIWI_QWENCODE_RUNTIME_DIR%"
if "%RUNTIME%"=="" set "RUNTIME=D:\aiops\qwencode"
call "%ROOT%\check-qwencode-runtime.cmd"
if errorlevel 1 exit /b %ERRORLEVEL%
set "PATH=%RUNTIME%;%RUNTIME%\runtimes\node;%PATH%"
set "PYTHON_CMD=python"
set "AIOPS_DOCS_DIR=%KIWI_AIOPS_DOCS_DIR%"
if "%AIOPS_DOCS_DIR%"=="" set "AIOPS_DOCS_DIR=D:\aiops\docs"

if not exist "%AIOPS_DOCS_DIR%" mkdir "%AIOPS_DOCS_DIR%"
if exist "%ROOT%\docs\project-knowledge-packs\v1" (
  for /d %%P in ("%ROOT%\docs\project-knowledge-packs\v1\*") do (
    if exist "%%~fP\docs\knowledge\00-index.md" (
      if not exist "%AIOPS_DOCS_DIR%\%%~nxP" mkdir "%AIOPS_DOCS_DIR%\%%~nxP"
      robocopy "%%~fP\docs\knowledge" "%AIOPS_DOCS_DIR%\%%~nxP\knowledge" /E /NFL /NDL /NJH /NJS /NC /NS >nul
      if errorlevel 8 exit /b %ERRORLEVEL%
    )
  )
)
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  "[Environment]::SetEnvironmentVariable('KIWI_AIOPS_DOCS_DIR',$env:AIOPS_DOCS_DIR,'User')"
if errorlevel 1 exit /b %ERRORLEVEL%
...
"%ROOT%\.venv\Scripts\python.exe" -m pip install --no-index --find-links "%ROOT%\vendor\python-wheelhouse" --upgrade --force-reinstall pywinpty==3.0.3
if errorlevel 1 exit /b %ERRORLEVEL%

"%ROOT%\.venv\Scripts\python.exe" -c "from winpty import PtyProcess; print('pywinpty OK')"
if errorlevel 1 (
  echo [ERROR] pywinpty import failed in KIWI .venv. Delete "%ROOT%\.venv" and run install-offline.cmd again.
  exit /b %ERRORLEVEL%
)
...
"%RUNTIME%\runtimes\node\npm.cmd" run build
if errorlevel 1 exit /b %ERRORLEVEL%
if exist "%ROOT%\kiwi-source-stamp.txt" (
  if not exist "%ROOT%\.next" mkdir "%ROOT%\.next"
  copy /y "%ROOT%\kiwi-source-stamp.txt" "%ROOT%\.next\kiwi-source-stamp.txt" >nul
)

echo.
echo KIWI offline install complete.
echo External qwencode runtime: %RUNTIME%
echo Reopen terminal, then qwen-init.cmd can be called from any project path.
echo Run start-kiwi.cmd for the web wrapper.
""".strip()
        + "\n",
    )
    write_crlf(
...
call "%ROOT%\check-qwencode-runtime.cmd"
if errorlevel 1 exit /b %ERRORLEVEL%
set "PATH=%ROOT%\bin;%RUNTIME%;%RUNTIME%\runtimes\node;%PATH%"
set "NEXT_PUBLIC_KIWI_API_URL=http://localhost:8787"
cd /d "%ROOT%"
set "KIWI_STOP_PORTS=3000,8787"
...
set "NEEDS_BUILD=0"
if not exist "%ROOT%\.next\BUILD_ID" set "NEEDS_BUILD=1"
if not exist "%ROOT%\.next\kiwi-source-stamp.txt" set "NEEDS_BUILD=1"
if exist "%ROOT%\kiwi-source-stamp.txt" (
  fc /b "%ROOT%\kiwi-source-stamp.txt" "%ROOT%\.next\kiwi-source-stamp.txt" >nul 2>nul
  if errorlevel 1 set "NEEDS_BUILD=1"
)
if "%NEEDS_BUILD%"=="1" (
  echo [INFO] Next production build is missing or stale. Rebuilding before KIWI starts...
...
if errorlevel 1 exit /b %ERRORLEVEL%
set "PATH=%ROOT%\bin;%RUNTIME%;%RUNTIME%\runtimes\node;%PATH%"
set "KIWI_STOP_PORTS=8787"
call "%ROOT%\stop-kiwi.cmd"
if errorlevel 1 exit /b %ERRORLEVEL%
"%ROOT%\.venv\Scripts\python.exe" -c "from winpty import PtyProcess; print('pywinpty OK')"
if errorlevel 1 (
  echo [ERROR] pywinpty import failed in KIWI .venv. Run install-offline.cmd again.
  exit /b %ERRORLEVEL%
)
...
if "%NEEDS_BUILD%"=="1" (
  echo [INFO] Next production build is missing or stale. Rebuilding with offline npm cache...
  if exist "%ROOT%\.next" rmdir /s /q "%ROOT%\.next"
  "%RUNTIME%\runtimes\node\npm.cmd" run build
  if errorlevel 1 exit /b %ERRORLEVEL%
  if exist "%ROOT%\kiwi-source-stamp.txt" (
...
"%ROOT%\.venv\Scripts\python.exe" -c "from winpty import PtyProcess; from app.main import app; from app.qwencode_runtime import find_latest_qwencode_runtime; print('pywinpty OK'); print(app.title); print(find_latest_qwencode_runtime())"
if errorlevel 1 exit /b %ERRORLEVEL%
...
- `qwen.cmd`, `bin/qwen.cmd`, `bin/qwencode.cmd`: 외부 `D:\aiops\qwencode\run-qwen.cmd` 래퍼
- `qwen-init.cmd`, `bin/qwen-init.cmd`: 외부 `D:\aiops\qwencode\qwen-init.cmd` 래퍼

## 설치

1. `qwencode-win11-v0.17.1.zip`을 `D:\aiops`에 풀어 `D:\aiops\qwencode`를 만든다.
2. `D:\aiops\qwencode\install-path.cmd`를 실행한 뒤 새 터미널을 연다.
3. `kiwi-offline-win11-py313.zip`을 `D:\aiops`에 풀어 `D:\aiops\kiwi`를 만든다.
4. Windows 11에 Python 3.13.5를 설치하고 `python`이 PATH에서 실행되게 한다.
5. KIWI 폴더에서 실행한다.
...
`install-offline.cmd`는 KIWI 안에 Qwen runtime을 설치하지 않는다. 대신 `D:\aiops\qwencode`의 `run-qwen.cmd`, `qwen-init.cmd`, `runtimes\node\npm.cmd`를 검증하고, 사용자 환경변수 `KIWI_QWENCODE_RUNTIME_DIR`와 사용자 `PATH`에 `D:\aiops\qwencode`를 등록한다.
새 터미널을 열면 어느 프로젝트 폴더에서든 `qwen-init.cmd`, `qwen.cmd`, `qwencode.cmd`를 바로 실행할 수 있다.
...
- `start-kiwi.cmd`는 먼저 3000/8787 포트의 기존 KIWI 프로세스를 정리한 뒤 시작한다.
- `.next` production build가 없거나 `kiwi-source-stamp.txt`가 맞지 않으면 `start-kiwi.cmd`가 기존 `.next`를 삭제하고 오프라인 캐시로 `npm run build`를 완료한 뒤 Backend/Web을 시작한다.
- `run-web.cmd`를 직접 실행해도 `.next`가 없거나 stale이면 build 후 start까지 이어서 수행한다.
...
Windows에서 `D:\aiops\qwencode\run-qwen.cmd`가 있으면 KIWI는 이 standalone runtime을 우선 사용한다.
KIWI 번들에는 Qwen runtime을 중복 포함하지 않으므로, 해당 경로가 없으면 설치와 실행은 명확히 실패한다.
`qwen.cmd`와 `qwen-init.cmd` wrapper도 같은 외부 runtime을 사용한다.
...
정상 설치 후 KIWI 웹을 거치지 않고 터미널에서 바로 쓸 수 있다.
...
- Explorer-35: `{ORCHESTRATOR_API_BASE_URL}`, `{ORCHESTRATOR_MODEL}`
...
def write_manifest(target_root: Path, runtime: Path) -> None:
    runtime_package = runtime / "app" / "package.json"
    runtime_version = ""
    if runtime_package.exists():
        runtime_version = json.loads(runtime_package.read_text(encoding="utf-8")).get("version", "")
    manifest = {
        "name": KIWI_ROOT_NAME,
        "zipName": f"{KIWI_ZIP_NAME}.zip",
        "pairedQwencodeZip": f"{QWENCODE_ZIP_NAME}.zip",
        "python": "3.13.x win_amd64 cp313",
        "qwenRuntime": runtime.name,
        "qwenRuntimeSource": str(runtime),
        "qwenCodeVersion": runtime_version,
        "orchestratorApiBaseUrl": ORCHESTRATOR_API_BASE_URL,
        "orchestratorModel": ORCHESTRATOR_MODEL,
        "orchestratorModalities": {"image": True},
        "orchestratorSplitToolMedia": True,
        "coderApiBaseUrl": CODER_API_BASE_URL,
        "coderModel": CODER_MODEL,
        "workModes": ["fast", "ultrawork", "superpowers"],
        "projectDocsRoot": "D:\\aiops\\docs",
        "projectDocsLayout": {
            "projectInfo": "D:\\aiops\\docs\\<project-key>\\project-info",
            "knowledge": "D:\\aiops\\docs\\<project-key>\\knowledge",
        },
        "runtimeBundled": False,
        "externalQwencodeRuntimeRequired": True,
        "runtimePriority": ["D:\\aiops\\qwencode", "KIWI_QWENCODE_RUNTIME_DIR"],
        "fastSystemPrompts": "D:\\aiops\\qwencode\\portable-user\\.qwen\\extensions\\fast-system-prompts",
        "superpowersExtension": "D:\\aiops\\qwencode\\portable-user\\.qwen\\extensions\\superpowers",
        "superpowersSkills": superpowers_skill_names(),
        "explorerAgent": "explorer-35",
        "explorerApiBaseUrl": ORCHESTRATOR_API_BASE_URL,
        "explorerModel": ORCHESTRATOR_MODEL,
        ...
        "standaloneQwencodeRuntimeDir": DEFAULT_QWENCODE_RUNTIME_DIR,
    }
