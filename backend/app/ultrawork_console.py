...
from .work_modes import (
    DEFAULT_TASK_SIZE,
    WorkMode,
    detect_work_mode_trigger_with_size,
    ensure_work_mode_prefix,
    normalize_work_mode,
    normalize_task_size,
    prefix_for_work_mode,
    strip_work_mode_trigger,
    work_mode_definition,
)
...
# KIWI patches that guard out of the runtime (see _patch_console_paste_guard);
# the timing-based delay below stays only as a fallback for unpatched runtimes.
QWEN_PASTE_GUARD_SECONDS = 0.62
QWEN_PASTE_GUARD_BYPASS_MARKER = 'process.env["KIWI_ULTRAWORK_CONSOLE"] !== "1"'
PASTE_SUBMIT_BYPASS_SECONDS = 0.05
PTY_WRITE_CHUNK_CHARS = 4096
PASTE_SUBMIT_CHARS_PER_SECOND = 32_000
PASTE_SUBMIT_LINE_SECONDS = 0.002
DIRECT_ULTRAWORK_MARKER = "[KIWI_DIRECT_ULTRAWORK_SIZING_GATE]"
WORK_MODE_LOCK_MARKER = "[KIWI_WORK_MODE_LOCK]"
...
class ConsoleSession:
    ...
    work_mode_activated: bool = False
    paste_guard_bypassed: bool = False
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "work_mode_label": work_mode_definition(self.work_mode).label,
            "work_mode_prefix": prefix_for_work_mode(self.work_mode, self.task_size),
            "task_size": self.task_size,
            "work_mode_locked": True,
            "work_mode_activated": self.work_mode_activated,
...
class UltraworkConsoleManager:
    ...
    async def start_session(
        ...
        task_size: str | None = None,
        task_size_reason: str | None = None,
        cols: int = 140,
        rows: int = 36,
    ) -> dict[str, Any]:
        settings = get_internal_settings()
        normalized_work_mode = normalize_work_mode(work_mode)
        normalized_task_size = None if normalized_work_mode == "fast" else (normalize_task_size(task_size) or DEFAULT_TASK_SIZE)
        _validate_start_prompt_task_size(normalized_work_mode, initial_prompt, task_size)
        ...
        if command and preferred_runtime and not project_runtime:
            raise HTTPException(
                status_code=409,
                detail=(
                    "프로젝트 qwen.cmd가 존재하지만 가리키는 run-qwen.cmd 경로를 찾을 수 없습니다. "
                    "왼쪽 프로젝트 초기화를 다시 실행해 qwen-init으로 qwen.cmd를 현재 KIWI 런타임 기준으로 재생성하세요. "
        ...
        command = _command_with_approval_mode(command, dangerous_mode=settings.dangerous_mode)
        ...
        state = ConsoleSession(
            ...
            task_size=normalized_task_size,
            paste_guard_bypassed=_runtime_paste_guard_bypassed(project_runtime or preferred_runtime),
            log_path=log_path,
        ...
        if initial_prompt and initial_prompt.strip():
            asyncio.create_task(self._delayed_input(state.id, initial_prompt, 0.8, normalized_task_size, task_size_reason))
    ...
    async def send_input(
        ...
        if submit:
            text = _prepare_submitted_work_mode_prompt(state, text, task_size, task_size_reason)
        if bracketed_paste:
            await self._write_bracketed_paste(state, text)
        else:
            await self._write_input(state, text)
        if submit:
            delay = _paste_submit_delay(text, state.paste_guard_bypassed) if bracketed_paste else 0
            if delay > 0:
    ...
    async def _spawn(
        ...
        if os.name == "nt":
            try:
                ...
                raise RuntimeError(
                    "Windows qwencode interactive console requires pywinpty. "
                    "Run install-offline.cmd again and verify `from winpty import PtyProcess` works in KIWI .venv. "
                    f"python={sys.executable}; import_error={exc!r}"
                ) from exc
    ...
    async def _delayed_input(
        ...
        if session_id in self._sessions:
            await self.send_input(
                ...
                task_size=task_size,
                task_size_reason=task_size_reason,
            )
...
def _command_with_approval_mode(command: list[str], dangerous_mode: bool) -> list[str]:
    if "--approval-mode" in command or any(part.startswith("--approval-mode=") for part in command):
        return command
    return [*command, "--approval-mode", "yolo"]


def _validate_start_prompt_task_size(work_mode: WorkMode, initial_prompt: str | None, task_size: str | None) -> None:
    if work_mode == "fast":
        if task_size:
...
def _prepare_submitted_work_mode_prompt(
    state: ConsoleSession,
    text: str,
    task_size: str | None = None,
    task_size_reason: str | None = None,
    ...
    if detected and detected[1] != state.work_mode:
        requested = work_mode_definition(detected[1]).label
        locked = work_mode_definition(state.work_mode).label
        raise HTTPException(
            status_code=409,
            detail=(
                "현재 콘솔 세션의 work mode는 "
                f"{locked}로 잠겨 있습니다. {requested}로 바꾸려면 이 콘솔을 종료하고 새 세션을 시작하세요."
            ),
        )
    if state.work_mode_activated:
        return text
    detected_size = detected[2] if detected else None
    source_task_size = normalize_task_size(task_size) or detected_size or normalize_task_size(state.task_size) or DEFAULT_TASK_SIZE
    state.work_mode_activated = True
    if state.work_mode != "fast":
        state.task_size = source_task_size
    prefixed = ensure_work_mode_prefix(text, state.work_mode, source_task_size)
    canonical_prefix = prefix_for_work_mode(state.work_mode, source_task_size if state.work_mode != "fast" else None)
    task = strip_work_mode_trigger(prefixed).strip()
    prefixed = "\n\n".join([part for part in [canonical_prefix, task] if part]).strip()
    if state.work_mode == "fast":
        return prefixed

    if not _has_tshirt_section(task):
        return prefixed

    policy = build_ultrawork_policy(
        {"name": state.project_name, "root_path": state.root_path},
        _direct_ultrawork_intent(task, task_size=source_task_size, task_size_reason=task_size_reason),
        selected_task_size=source_task_size,
        selected_task_size_reason=task_size_reason,
    )
    policy = {**policy, "work_mode": state.work_mode}
    replaced_task = _replace_tshirt_section(task, render_tshirt_section(policy))
    result = "\n\n".join(
        [
            prefix_for_work_mode(state.work_mode, source_task_size),
            replaced_task or "사용자가 작업 내용을 쓰지 않고 work mode prefix만 입력했다. 먼저 `ask_user_question`으로 실제 작업 내용을 확인한다.",
        ]
    ).strip()
    return result


def _inject_direct_ultrawork_sizing_gate(state: ConsoleSession, text: str) -> str:
    if DIRECT_ULTRAWORK_MARKER in text:
        return text

    match = re.match(r"^\s*((?:ultrawork|ulw)(?:_(?:xsmall|small|medium|large|xlarge))?)(?:\s*(?:\r?\n|$))", text, flags=re.IGNORECASE)
    if not match:
        return text
    if "## 티셔츠 사이징" in text[:3000]:
        return text

    trigger = match.group(1).lower()
    _base_trigger, trigger_size = trigger.rsplit("_", 1) if "_" in trigger else (trigger, "")
    selected_size = normalize_task_size(trigger_size) or normalize_task_size(state.task_size) or DEFAULT_TASK_SIZE
    task = text[match.end() :].lstrip("\r\n")
    policy = build_ultrawork_policy(
        {"name": state.project_name, "root_path": state.root_path},
        _direct_ultrawork_intent(task, task_size=selected_size, task_size_reason=f"사용자가 `{selected_size}`를 선택했다."),
    ...
    lines = [
        trigger,
        "",
        DIRECT_ULTRAWORK_MARKER,
        "아래는 KIWI 웹 콘솔이 direct ultrawork 입력에 자동 주입한 선행 sizing gate다.",
        "어떤 `agent` tool 호출도 하기 전에 먼저 사용자에게 `## 티셔츠 사이징` 결과와 사용자 선택 규모별 실행 계획을 한국어로 보고한다.",
        "특히 explorer-35, planner-35, architect-35, coder-35 호출은 이 보고 이후에만 가능하다.",
        "영향 범위가 선택 size보다 커지면 현재 세션에서 임의로 바꾸지 말고 더 큰 size의 새 콘솔을 시작하라고 보고한다.",
        "",
        *render_tshirt_section(policy),
        "",
        "## direct ultrawork subagent gate",
        ...
        *_bullet_lines(render_qwencode_tool_cheatsheet()),
        "",
        "## 원 사용자 요청",
        task.strip() or "사용자가 작업 내용을 쓰지 않고 ultrawork만 입력했다. 먼저 `ask_user_question`으로 실제 작업 내용을 확인한다.",
    ]
    return "\n".join(lines).strip()


def _direct_ultrawork_intent(
    task: str,
    task_size: str | None = None,
    task_size_reason: str | None = None,
) -> dict[str, Any]:
    return {
        "task_summary": task[:4000],
        "task_type": "unknown",
        **({"task_size": task_size} if task_size else {}),
        **({"task_size_reason": task_size_reason} if task_size_reason else {}),
        "target_files": _path_like_tokens(task),
        "search_queries": _direct_search_queries(task),
        "risk_flags": [],
    }


def _has_tshirt_section(text: str) -> bool:
    return "## 티셔츠 사이징" in text[:6000]


def _extract_tshirt_size(text: str) -> str | None:
    head = text[:6000]
    if "## 티셔츠 사이징" not in head:
        return None
    if "최종 source of truth: 사용자 선택값" not in head:
        return None
    match = re.search(r"사용자\s*선택:\s*`?(xsmall|small|medium|large|xlarge)`?", head)
    if match:
        return match.group(1)
    return None


def _replace_tshirt_section(text: str, section_lines: list[str]) -> str:
...
def _direct_search_queries(task: str) -> list[str]:
    words = [word for word in re.split(r"[\s,.;:/()\[\]{}<>\"'`]+", task) if len(word) >= 2]
    preferred = [word for word in words if re.search(r"[A-Za-z0-9가-힣_-]", word)]
    return preferred[:8] or [task[:120]]


def _path_like_tokens(task: str) -> list[str]:
    tokens = re.findall(r"[\w./\\-]+\.(?:js|ts|tsx|vue|java|xml|yml|yaml|json|md|sql|properties)", task, flags=re.IGNORECASE)
    return tokens[:12]
...
def _paste_submit_delay(text: str, paste_guard_bypassed: bool = False) -> float:
    if paste_guard_bypassed:
        # PTY input is ordered, so the submit CR is processed after the paste;
        # with the runtime guard patched out no further settling time is needed.
        return PASTE_SUBMIT_BYPASS_SECONDS
    normalized = text.replace("\r\n", "\n").replace("\r", "\n")
    return (
...
def _runtime_paste_guard_bypassed(runtime: Path | None) -> bool:
    if runtime is None:
        return False
    cli_path = runtime / "app" / "cli.js"
    try:
        return QWEN_PASTE_GUARD_BYPASS_MARKER in cli_path.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return False
